...
const srv = spawn('node', [resolve(HERE, 'server.mjs')], { stdio: ['pipe', 'pipe', 'inherit'] });
...
assert.deepEqual(tools.sort(), ['apply', 'exec', 'export_m', 'notebook', 'read_variable']);
pass(`MCP handshake, 5 tools: ${tools.join(' ')}`);
...
assert.ok(m.source.includes('%% cell 1') && m.source.includes('f = 7;'));
pass('export_m: dependency-ordered plain MATLAB with the applied edit');
